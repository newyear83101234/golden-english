...
Golden（HUNTR/X）副歌逐字同步亮字 → 點任何單字：歌暫停＋彈學習卡（原文 TTS 發音 → 中文意思語音；注音直排在國字右側、台灣課本格式、標本調）→「歌裡怎麼唱」重播該字歌曲片段 →「繼續唱歌」從該字前 0.8 秒續播。新單字集星星（localStorage）。**韓文行也可學**（2026-07-28 阿葉指示）：拆 3 個單字（영원히/깨질/수 없는），紫色系＋「韓文」標籤，TTS 用 ko-KR-SunHiNeural。
